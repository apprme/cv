...
resource "aws_s3_bucket_object" "object" {
  ...
  content_type        = length(regexall(local.pdf, each.value)) > 0 ? "application/pdf" : length(regexall(local.txt, each.value)) > 0 ? "text/plain" : null
  content_disposition = length(regexall(local.pdf, each.value)) > 0 ? "inline; filename=\"${basename(each.value)}\"" : null
}
